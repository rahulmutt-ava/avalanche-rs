...
> **Status (2026-06-05, M0 COMPLETE):** All M0 tasks (M0.1–M0.25) are landed and merged to
...
> M0.19, M0.20, M0.21); `ava-version` fully (M0.22, M0.23); **per-crate proptest contracts
> (M0.24)** and the **milestone exit gate (M0.25)**. The `avalanchers --version`
...
> secp_recover, nodeid_from_cert, sampler_mt19937_stream}`, `prop::codec_roundtrip` (cases=4096),
> `conformance::run_codec_suite`. Workspace: **124 tests pass** (`--all-features`), `cargo build
> --workspace`, `cargo build -p avalanchers`, `cargo clippy --workspace --all-targets -- -D warnings`,
> and `cargo fmt --all -- --check` all clean. **M0 is buildable-&-green; ready to begin M1 (storage).**
>
> **M0.24 build (2026-06-05, parallel subagents in isolated worktrees, merged to `main`):**
> - Each T0 crate gained `tests/proptests.rs`: `ava-codec` (`prop::codec_roundtrip` @ cases=4096
>   + `prop::decode_never_panics` + length-prefix bounds); `ava-types` (id/CB58 string + byte
>   round-trips, op purity); `ava-crypto` (secp sign→verify/tamper/low-S, BLS sign-verify +
>   aggregate==individual, address purity); `ava-utils` (safemath vs u128 oracle, `Bits` set
>   algebra vs `BTreeSet`, CB58 round-trip, sampler distinctness/count); `ava-version` (version
>   compare total-order, upgrade activation monotonicity + boundary-inclusivity).
> - Fuzz targets wired from no-op stubs to real bodies: `ava-codec/fuzz` `codec_roundtrip`
>   (decode-never-panics + re-marshal round-trip), `ava-utils/fuzz` `cb58_roundtrip`. **Note:**
>   `cargo +nightly fuzz build` could NOT be exercised — no nightly toolchain in the dev/sandbox
>   environment; targets are type-checked against real public API via stable `cargo check` only.
>   **Owed to X-cross-cutting:** run `cargo +nightly fuzz build` in CI.
> - **Keys lack `Debug`** (`secp256k1::PrivateKey`, `bls::SecretKey`, `UpgradeConfig`), which
>   proptest requires for shrinkable case inputs. Resolved by generating from `Debug` seeds
>   (`[u8;32]` / network-id `u32`) and constructing the non-`Debug` value *inside* the test body.
> - **WWR distinctness correction** (`ava-utils`): the spec phrase "without replacement never
>   repeats" is **false** for `WeightedWithoutReplacement` (draws over weight space → may repeat an
>   index, matching Go + golden `samplers.json`); distinctness only holds for the *uniform* sampler.
>   `specs/02 §4.2` updated to record this; the proptest tests distinctness on `UniformReplacer`
>   and count+range on the weighted sampler.
> - **Detached fuzz `Cargo.lock`** files (generated by `cargo check` on the fuzz sub-crates) are
>   left untracked (no fuzz lockfile is tracked or gitignored repo-wide; matches existing convention).
>
...
### Task M0.24: Per-crate contracts — proptest suites, regressions, fuzz targets, PORTING.md ✅ COMPLETED
...
### Task M0.25: Milestone exit gate ✅ COMPLETED
