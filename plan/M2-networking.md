...
>
> **M2.20b ✅ DONE (2026-06-07).** Live increments wired at their call sites: `times_connected`/`times_disconnected` (`NetworkImpl::watch_peer`), `msgs{io=sent/received}` + `failed_to_send`/`failed_to_parse` (peer write/read tasks), `byte_throttler_inbound_remaining_*` (`InboundMsgByteThrottler::set_metrics`+`publish_remaining`), `tls_conn_rejected` (accept-path upgrade-`Err` arm — rustls verifier callback can't reach a `Metrics` handle, so metered one level up where the reject surfaces, matching Go). APIs kept stable via additive builders (`PeerConfig::with_peer_metrics`, `NetworkImpl::new_with_metrics`, `InboundMsgByteThrottler::set_metrics`); both handles `Option`-typed (no-op without a registry). **55 ava-network tests** incl. 2 new live-increment assertions (`tests/metrics_increments.rs`). **Still registered-but-unincremented (documented, not a gap):** `num_useless_peerlist_bytes` (needs an `IpTracker` per-claim useless-byte delta), the `peers`/uptime/outbound-throttler `awaiting_*` gauges whose runtime surfaces aren't in-crate yet.
