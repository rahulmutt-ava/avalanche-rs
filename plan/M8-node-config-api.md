...
> **Extended [UPSTREAM DELTA — 2026-07-20] (Go `7d8721750b`, #5678; `13` §staking-economics upstream-delta).** Defaults become **network-dependent**: `min-stake-duration` = `336h` Mainnet / `24h` Testnet; `helicon-min-stake-duration` = `48h` Mainnet / `1h` Testnet. The local-network staking-config validation gains two checks: `HeliconMinStakeDuration > 0` (`errInvalidHeliconMinStakeDuration`) and `MaxStakeDuration >= HeliconMinStakeDuration` (`errHeliconMinStakeDurationAboveMax`). Include both in the Rust decode + defaults table.
