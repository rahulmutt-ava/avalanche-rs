...
- [ ] **Step 1 — Red:** Add `golden::calculate_price` (the 9-row 21 §0 table incl. the `MaxUint64 − 11` row and the clamp row) and `golden::pchain_dynamic_fee` (the 21 §1 worked examples: excess=0 → fee=6600; excess=K → fee=13200; advance-then-consume round trip). Add `prop::price_monotone` (non-decreasing in excess; == minPrice at 0; never panics; ≤ MaxU64).
- [ ] **Step 2 — Confirm red:** `cargo nextest run -p ava-platformvm golden::calculate_price` → fails.
- [ ] **Step 3 — Green:** Implement `calculate_price(min_price, excess, k)` over `ruint::U256` EXACTLY per 21 §0 (clamp test `output >= max_output`; two separate divides `/k` then `/i`; trailing `/k`; m=0→0). `GasState { capacity, excess }` with `advance_time`/`consume_gas` (21 §1). `dot_to_gas(c, w)` checked dot product. Constants (mainnet=Fuji): weights `[1,1000,1000,4]`, `MaxCapacity=1_000_000`, `MaxPerSecond=100_000`, `TargetPerSecond=50_000`, `MinPrice=1`, `K=2_164_043`. Static `SimpleCalculator` returns flat `TxFee=MilliAvax=1_000_000`, `CreateAssetTxFee=10·MilliAvax` (Fuji), per-network from config. `complexity.rs` computes the `[Bandwidth,DBRead,DBWrite,Compute]` 4-vector per tx type (`txs/fee/complexity.go`). Fee path selects static (pre-Etna) vs dynamic (post-Etna) by fork.
- [ ] **Step 4 — Confirm green:** `cargo nextest run -p ava-platformvm golden::calculate_price golden::pchain_dynamic_fee prop::price_monotone` green.
- [ ] **Step 5 — Commit:** `ava-platformvm: ACP-103 dynamic fee + static fee + complexity`
