...
### Task M9.19: `test-reexecute` — replay recorded mainnet ranges → state roots match Go
...
- [ ] **Step 1 — Red:** Write `reexecute_cchain_range` and `reexecute_px_range`: from a fixed starting state, replay a recorded range of mainnet C-Chain (and P/X) blocks (`blockexport` fixtures) through the Rust VMs; assert resulting state/merkle roots match the Go-recorded expected roots byte-for-byte (a differential oracle on recorded data). Add `cargo xtask test-reexecute`.
- [ ] **Step 2 — Confirm red:** `cargo xtask test-reexecute` → fails.
- [ ] **Step 3 — Green:** Implement the reexecution harness consuming Go `blockexport` artifacts (reuse the M6 reexecute fixtures); a fixed-start-state replay loop per chain asserting roots; wire the `xtask` alias. Mark it as the per-PR cheap differential oracle (`00` §11.7).
- [ ] **Step 4 — Confirm green:** `cargo xtask test-reexecute` → passes (per-PR budget).
- [ ] **Step 5 — Commit:** `tests: test-reexecute recorded mainnet ranges → Go-identical state roots`
...
### Task M9.21: `bench-guard` perf gates
...
- [ ] **Step 1 — Red:** Write `bench_guard_holds`: run the critical-path criterion benches (codec encode/decode, merkledb commit, signature verify, mempool push/pop, message framing, plus the M9 hot paths — rpcchainvm RPC round-trip) and assert each is within threshold (default 10%) of the committed baseline; a synthetic regressed bench must make the guard **fail** (proves the gate works).
- [ ] **Step 2 — Confirm red:** `cargo xtask bench-guard` → fails (no baseline / guard logic absent).
- [ ] **Step 3 — Green:** Implement `bench_guard.rs` (criterion `--save-baseline`/comparison, per-bench threshold); commit baselines under `.config/criterion-baseline/`; ensure every `00` §9 optimization that shipped (zero-copy block bytes, parallel sig recovery, sharded mempool, channel reuse, arc-swap caches) shows a bench win **and** is backed by a passing differential test (cross-link M9.15/M9.19/M9.20).
