...
> **FOLLOW-UPS:** (1) ✅ **RESOLVED (cchain-tx-pipeline task 5).** The "wire reth `best_transactions` when the EVM txpool lands" reading is **retired**: ava-evm never adopts reth's `TransactionPool`. Instead a **purpose-built `EvmMempool`** (`crates/ava-evm/src/mempool.rs`, design doc 2026-07-17, mirroring coreth's submission-path admission rules) supplies the fee-cap-ordered candidates. `vm.rs::build_block` now drains `EvmMempool::best_txs()` into `build_on` (snapshotting `(signer, nonce, hash)` for whole-batch eviction on a non-`MissingProposal` execution failure), `wait_for_event` wakes on the pool's admission notify (notify-driven `PendingTxs`), and `VerifiedEvmBlock::accept` runs `on_block_accepted` maintenance over the txs actually in the accepted block. (2) `vm.rs::build_block` uses `AvaFeeState::default()` for the next-block ctx — thread parent-extra-data fee-state extraction (AP3 window blob / ACP-176 24-byte state) once M6.7's extractor is wired. (3) ✅ **RESOLVED:** `build_on` now derives real `tx_root`/`receipt_root`/`bloom` from the executed body (`builder.rs:261-275`), and per-tx receipts are stashed at verify time then persisted into the `CanonicalStore` + `AcceptedTxIndex` at accept (cchain-tx-pipeline task 3 — the old `append_canonical(..., &[])` empty-receipts placeholder is gone).
