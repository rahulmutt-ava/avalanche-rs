...
| `ava-utils::sampler` | weighted sampler reproduces Go's distribution for a fixed seed; sample count == requested. **Distinctness caveat (M0.24 finding):** "without replacement never repeats" holds for the *uniform* sampler (`UniformReplacer`, draws over slot space → distinct indices), but **NOT** for `WeightedWithoutReplacement`, which draws without replacement over the *weight space* and intentionally returns the same index multiple times when its weight band is selected more than once (matches Go; see golden `tests/vectors/sampler/samplers.json`, e.g. `[1,1,4]`). Test distinctness on the uniform sampler; test count+valid-range on the weighted one. |
