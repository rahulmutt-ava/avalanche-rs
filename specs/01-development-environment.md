...
|---|---|---|
| `bazel build //crates/ava-node:avalanchers` | `cargo build -p avalanchers --release` | `rust_binary` target generated by gazelle. |
...
|---|---|---|
| `build` | `build` | `cargo build -p avalanchers --release` |
...
tasks:
  ...
  bazel-build:
    ...
    cmd: '{{.NIX_RUN}} bazelisk build //crates/ava-node:avalanchers'

  bazel-build-opt:
    desc: Build avalanchers with Bazel (optimized)
    cmd: '{{.NIX_RUN}} bazelisk build --config=opt //crates/ava-node:avalanchers'
