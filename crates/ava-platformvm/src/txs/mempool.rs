...
//! The P-Chain transaction mempool (`vms/txs/mempool/mempool.go`, specs 08
//! §4.3).
//!
//! A port of the generic `vms/txs/mempool` FIFO pool (not the Etna gas-priced
//! `vms/platformvm/txs/mempool` variant, which is gas-aware block packing — out
//! of scope for read-only sync). The behavior matched here is:
//!
//! - **Insertion-ordered (FIFO)** — [`Mempool::add`] appends; [`Mempool::peek`]
//!   returns the oldest tx; [`Mempool::iterate`] / [`Mempool::snapshot`] walk in
//!   oldest → newest order. This is the order the block builder (M4.25
//!   `build_block`) packs decision txs in.
...
//!   position. Mirrors Go's `Add` rejecting before `linked.Hashmap.Put`.
...
//!   add that would exceed the byte budget is rejected with
//!   [`Error::MempoolFull`] (drop-on-full — the pool never evicts an existing
//!   tx to make room, matching the generic Go base mempool).
//! - **Conflict-free** — a tx whose consumed UTXO inputs overlap a tx already in
//!   the pool is rejected with [`Error::ConflictsWithOtherTx`] (matches Go
//!   `consumedUTXOs.HasOverlap`).
//!
//! `add` / `remove` are idempotent in the property sense: removing every tx that
//! was added (and nothing else) returns the pool to empty with no tx lost or
//! duplicated (`prop::mempool_no_loss`).
...
/// module because they are not P-Chain consensus errors; the gossip handler maps
/// them to "drop, no divergence".
...
/// `mempool[T]` — an insertion-ordered, byte-bounded, conflict-free tx pool
/// (specs 08 §4.3).
///
...
mod conformance {
    ...
    /// TDD ENTRY POINT (M4.26). FIFO order, dedupe by tx ID, and drop-on-full at
    /// the byte capacity bound.
    #[test]
    fn mempool_dedupe_fifo() {
        ...
        oversized.bytes = vec![0u8; MAX_TX_SIZE + 1];
