...
//!
//! > **Note:** Go puts this in `utils/bloom`. Originally ported to `ava-network`
//! > (M2.17) for the handshake milestone; hoisted here to `ava-utils` so
//! > VM-layer code (e.g., tx gossip) can depend on the bloom filter writer
//! > without pulling the networking stack.
...
pub fn hash(key: &[u8], salt: &[u8]) -> u64 {
    // SHA256(key || salt) via sha2 crate (identical to Go's `sha256.New().Write(key);
    // Write(salt)`).
