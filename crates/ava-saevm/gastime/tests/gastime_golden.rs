...
fn vector1_price_at_excess_zero_and_e() {
    ...
    let tm = GasTime::from_excess(0, TARGET, 0, default_cfg());
    ...
    let tm = GasTime::from_excess(0, TARGET, K, default_cfg());
...
fn vector2_tick_accrual() {
    ...
    let mut tm = GasTime::from_excess(0, TARGET, 0, default_cfg());
...
fn vector3_fast_forward_decay() {
    ...
    let mut tm = GasTime::from_excess(0, TARGET, 1_000_000, default_cfg());
...
fn price_floor_respects_min_price() {
    ...
    let tm = GasTime::from_excess(0, TARGET, 0, cfg);
...
fn price_matches_calculate_price_rows() {
    ...
    for &(excess, _label) in &[(0u64, "e^0"), (K, "e^1"), (2 * K, "e^2")] {
        let tm = GasTime::from_excess(0, TARGET, excess, default_cfg());
